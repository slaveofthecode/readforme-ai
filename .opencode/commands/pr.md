---
description: Create a GitHub PR from the current branch targeting staging
---
...
3. Auto-generate the PR description from the branch changes:
   - Run `git log staging..HEAD --oneline --no-decorate` to list commits
   - Run `git diff staging..HEAD --stat` to list changed files
...
5. Create the PR with `gh pr create --base staging --title "<branch>" --body "<description>" --assignee "<user>"`
